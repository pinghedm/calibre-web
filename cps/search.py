...
def advanced_search():
    ...
    search_params = json.dumps({k: v for k, v in values.items() if k != "csrf_token"})
    return redirect(
        url_for(
            "web.books_list",
            data="advsearch",
            ...
            if custom_start:
                q = q.filter(
                    getattr(db.Books, "custom_column_" + str(c.id)).any(
                        func.datetime(db.cc_classes[c.id].value)
                        >= func.datetime(custom_start)
                    )
                )
            if custom_end:
                q = q.filter(
            ...
            if c.datatype == "bool":
                if custom_query != "Any":
                    if custom_query == "":
                        q = q.filter(
                            ~getattr(db.Books, "custom_column_" + str(c.id)).any(
                                db.cc_classes[c.id].value >= 0
                            )
                        )
                    else:
                        q = q.filter(
                            getattr(db.Books, "custom_column_" + str(c.id)).any(
                                db.cc_classes[c.id].value
                                == bool(custom_query == "True")
                            )
                        )
                ...
                if c.datatype == "rating":
                    ...
                    q = q.filter(
                        getattr(db.Books, "custom_column_" + str(c.id)).any(
                            func.lower(db.cc_classes[c.id].value).ilike(
                                "%" + custom_query + "%"
                            )
                        )
                    )
...
def adv_search_read_status(read_status):
    if not config.config_read_column:
        ...
        try:
            if read_status == "":
                db_filter = (
                    coalesce(db.cc_classes[config.config_read_column].value, 2) == 2
                )
            else:
                db_filter = db.cc_classes[config.config_read_column].value == bool(
                    read_status == "True"
                )
...
def adv_search_shelf(q, include_shelf_inputs, exclude_shelf_inputs):
    q = q.outerjoin(ub.BookShelf, db.Books.id == ub.BookShelf.book_id).filter(
        or_(ub.BookShelf.shelf == None, ub.BookShelf.shelf.notin_(exclude_shelf_inputs))
    )
        ...
        try:
            searchterm.extend(
                [
                    _("Published after ")
                    + format_date(
                        datetime.strptime(pub_start, "%Y-%m-%d"), format="medium"
                    )
                ]
            )
        except ValueError:
        ...
        try:
            searchterm.extend(
                [
                    _("Published before ")
                    + format_date(
                        datetime.strptime(pub_end, "%Y-%m-%d"), format="medium"
                    )
                ]
            )
        except ValueError:
...
def render_adv_search_results(term, offset=None, order=None, limit=None):
    ...
    for c in cc:
                ...
                search_term.extend(
                    [
                        "{} >= {}".format(
                            c.name,
                            format_date(
                                datetime.strptime(column_start, "%Y-%m-%d").date(),
                                format="medium",
                            ),
                        )
                    ]
                ...
                search_term.extend(
                    [
                        "{} <= {}".format(
                            c.name,
                            format_date(
                                datetime.strptime(column_end, "%Y-%m-%d").date(),
                                format="medium",
                            ),
                        )
                    ]
        ...
        if c.datatype in ["int", "float"]:
            ...
            if term.get("custom_column_" + str(c.id)) != "Any":
                search_term.extend(
                    [("{}: {}".format(c.name, term.get("custom_column_" + str(c.id))))]
                )
                cc_present = True
    ...
    if (
        ...
        if author_name:
            q = q.filter(
                db.Books.authors.any(
                    func.lower(db.Authors.name).ilike("%" + author_name + "%")
                )
            )
        if book_title:
        ...
        if publisher:
            q = q.filter(
                db.Books.publishers.any(
                    func.lower(db.Publishers.name).ilike("%" + publisher + "%")
                )
            )
        q = adv_search_tag(q, tags["include_tag"], tags["exclude_tag"])
        q = adv_search_serie(q, tags["include_serie"], tags["exclude_serie"])
        q = adv_search_shelf(q, tags["include_shelf"], tags["exclude_shelf"])
        q = adv_search_extension(
            q, tags["include_extension"], tags["exclude_extension"]
        )
        ...
        if description:
            q = q.filter(
                db.Books.comments.any(
                    func.lower(db.Comments.text).ilike("%" + description + "%")
                )
            )
        ...
        try:
            ...
            flash(
                _("Error on search for custom columns, please restart Calibre-Web"),
                category="error",
            )
    ...
    entries = calibre_db.order_authors(
        q[offset:limit_all], list_return=True, combined=True
    )
    return render_title_template(
        "search.html",
